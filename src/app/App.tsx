...
import { DocumentImageOverlay } from './components/DocumentImageOverlay';
...
// ============================================================
function ProjectModalContent({ project: p, onClose, setPrototypeOverlay }: { project: Project; onClose: () => void; setPrototypeOverlay: (url: string | null) => void }) {
  ...
  // Resolve prototype URL from edits/overrides
  const resolvedPrototypeUrl = useMemo(() => {
    const key = `modal.project.${p.id}.prototypeUrl`;
    return edits[key]?.to ?? overrides[key] ?? p.prototypeUrl;
  }, [p.id, p.prototypeUrl, edits, overrides]);
  ...
  return (
    <>
      ...
      <div className="fs-content">
        ...
        <div className="fs-outcomes-header">
          <div className="fs-section-title"><E editKey={`modal.project.${p.id}.outcomes.heading`} value="Key Outcomes" /></div>
          {resolvedPrototypeUrl && (
            <button
  onClick={(e) => {
    e.stopPropagation();
    setPrototypeOverlay(resolvedPrototypeUrl ?? null);
  }}
  onMouseEnter={(e) => {
    const b = e.currentTarget
    b.style.background = 'var(--hoverFill, #f5f5f5)'
    b.style.boxShadow = '0 2px 6px rgba(0,0,0,0.25)'
  }}
  onMouseLeave={(e) => {
    const b = e.currentTarget
    b.style.background = 'var(--fill, #ffffff)'
...
>
  <E editKey={`modal.project.${p.id}.prototype.btn`} value={p.prototypeLabel} />
  ...
  >
    open_in_new
          ...
          {/* Hidden EditableText to register prototypeUrl for editing */}
          <span style={{ display: 'none' }}>
            <E editKey={`modal.project.${p.id}.prototypeUrl`} value={p.prototypeUrl} />
          </span>
        </div>
...
function StudyModalContent({ study: s, onClose, setDocumentOverlay }: { study: Study; onClose: () => void; setDocumentOverlay: (url: string | null) => void }) {
  const { edits, overrides } = useEditContext();

  // Resolve document image URL from edits → overrides → data.ts fallback
  const resolvedDocumentUrl = useMemo(() => {
    const key = `modal.study.${s.id}.documentUrl`;
    return edits[key]?.to ?? overrides[key] ?? s.documentUrl;
  }, [s.id, s.documentUrl, edits, overrides]);

  // Resolve button label from edits → overrides → data.ts fallback
  const resolvedDocumentLabel = useMemo(() => {
    const key = `modal.study.${s.id}.document.btn`;
    return edits[key]?.to ?? overrides[key] ?? s.documentLabel;
  }, [s.id, s.documentLabel, edits, overrides]);
      ...
      <div className="fs-content">
        ...
        <div className="fs-two-col">
          ...
          <div>
            ...
            {/* View Document button — always shown; URL editable per-study */}
            <div style={{ marginTop: 'var(--sp-20)' }}>
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setDocumentOverlay(resolvedDocumentUrl ?? null);
                }}
                onMouseEnter={(e) => {
                  const b = e.currentTarget;
                  b.style.background = 'var(--hoverFill, #f5f5f5)';
                ...
                style={{
                  ...
                  /* customizable variables — same palette as prototype button */
                  '--stroke': '#037E8A',
                  '--text': '#037E8A',
                  '--icon': '#037E8A',
                  '--fill': '#B3E5E8',
              ...
              >
                <E editKey={`modal.study.${s.id}.document.btn`} value={resolvedDocumentLabel} />
                <span
                  className="material-icons-outlined"
                  style={{ fontSize: 16, marginLeft: '2px', color: 'var(--icon)' } as React.CSSProperties}
                >
                  open_in_new
                </span>
              </button>

              {/* Hidden EditableText to register documentUrl for editing */}
              <span style={{ display: 'none' }}>
                <E editKey={`modal.study.${s.id}.documentUrl`} value={s.documentUrl} />
              </span>
            </div>
          </div>
        </div>
...
export default function App() {
  ...
  const [prototypeOverlay, setPrototypeOverlay] = useState<string | null>(null);
  const [documentOverlay, setDocumentOverlay] = useState<string | null>(null);
  ...
  const handleCopyState = useCallback(() => {
    const json = exportFullState(registryRef.current, edits, textOverrides, textDefaults);
  ...
  }, [edits, editCount, registrySize, showSnackbar]);
  ...
  // ── SEO: Update document.title and meta description as user scrolls sections ──
  useEffect(() => {
    const sectionMeta: Record<string, { title: string; description: string }> = {
      '': {
        title: 'Christopher Kenreigh — Director of UX & Product Design | USA · Remote-Ready',
        description: 'Christopher Kenreigh is a Director of UX & Product Design with 11+ years building design organizations and shipping measurable outcomes across e-commerce, enterprise, and industrial. Available for Director-level roles. USA-based.',
      },
      'featured-solutions': {
        title: 'Featured Solutions — Christopher Kenreigh, UX Director & Product Design Leader',
        description: 'UX and product design case studies by Christopher Kenreigh — AI Persona Agent, Design System Architecture, Membership UX, Live Metallurgy Dashboard, and Cart & Checkout optimization delivering measurable business outcomes.',
      },
      'experience': {
        title: 'Experience — Christopher Kenreigh, Director of UX & Product Design',
        description: '11+ years of design leadership across Plexus Worldwide, Freeport-McMoRan, Siemens, OpenTech Alliance, and Glynlyon. Strategy, team development, AI-augmented workflows, and business-outcome-driven UX.',
      },
      'studies': {
        title: 'Studies & Credentials — Christopher Kenreigh, UX Director',
        description: 'Education, professional certifications, and credentials of Christopher Kenreigh — Bachelor of Art, Microsoft, CompTIA, LambdaTest, NASBA, and LinkedIn Learning certifications across design, strategy, and technology.',
      },
      'about': {
        title: 'About — Christopher Kenreigh, Director of UX & Product Design',
        description: 'Christopher Kenreigh is a design executive and strategist with 11+ years building design organizations at the intersection of strategy, business goals, and human-centered product thinking. USA-based, remote-ready.',
      },
      'contact': {
        title: 'Contact — Christopher Kenreigh, UX Director | Available for Director Roles',
        description: 'Get in touch with Christopher Kenreigh — Director of UX & Product Design. Available for Director, Head of Design, and VP of Design roles. Email: c.kenreigh@gmail.com | LinkedIn: linkedin.com/in/kenreigh',
      },
    };
    const meta = sectionMeta[activeSection] || sectionMeta[''];
    ...
    <div ref={mainRef}>
        ...
        <section id="hero" className="parallax-section">
          ...
          <div className="hero-content">
            <div className="hero-left-col">
              ...
              {/* Hidden H1 — SEO/AEO: full keyword-rich identity for crawlers and AI engines. Visual design unchanged. */}
              <h1 style={{
                position: 'absolute', width: 1, height: 1,
                padding: 0, margin: '-1px', overflow: 'hidden',
                clip: 'rect(0,0,0,0)', whiteSpace: 'nowrap', border: 0,
              }}>
                Christopher Kenreigh — Director of UX &amp; Product Design | Digital Product Strategy, Design Leadership, Business Outcomes, Team Development | USA · Remote-Ready
              </h1>
      ...
      <div className={`fullscreen-modal ${modal ? 'open' : ''}`} role="dialog" aria-modal="true">
        <div className="fullscreen-modal-header">
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-12)' }}>
            <button className="fullscreen-modal-close" onClick={closeModal} aria-label="Back" style={{ width: 'auto', padding: '0 var(--sp-8)', gap: 'var(--sp-4)', fontSize: 'var(--type-label-large)', color: 'var(--md-on-surface-variant)' }}>
              ...
              <E editKey="modal.back" value="Projects" />
            </button>
          ...
          {(prototypeOverlay || documentOverlay) ? (
            <button
              className="fullscreen-modal-close"
              onClick={() => {
                if (prototypeOverlay) setPrototypeOverlay(null);
                if (documentOverlay) setDocumentOverlay(null);
              }}
        ...
        <div style={{ flex: 1, overflowY: 'auto', position: 'relative' }}>
          {modal?.type === 'project' && <ProjectModalContent project={projects[modal.id]} onClose={closeModal} setPrototypeOverlay={setPrototypeOverlay} />}
          {modal?.type === 'study' && <StudyModalContent study={studies[modal.id]} onClose={closeModal} setDocumentOverlay={setDocumentOverlay} />}
        </div>
        {prototypeOverlay && (
          <PrototypeOverlay
            url={prototypeOverlay}
            onClose={() => setPrototypeOverlay(null)}
          />
        )}
        {documentOverlay && (
